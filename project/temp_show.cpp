#include "temp_show.h"
#include "ui_temp_show.h"
#include <iostream>
#include <fstream>
#include <QFile>
#include <QTextStream>
using namespace std;
FILE *shomaresh_show_temp;
...
temp_show::temp_show(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::temp_show)
{
    ui->setupUi(this);
...
void temp_show::on_pushButton_clicked()
{

    int number_of_movies = 0;
    fopen_s(&shomaresh_show_temp, "shomaresh.txt", "ab");
    fclose(shomaresh_show_temp);
    fopen_s(&shomaresh_show_temp, "shomaresh.txt", "rb");
    if (!shomaresh_show_temp)exit(0);
    fread(&number_of_movies,sizeof(int),1,shomaresh_show_temp);
    fclose(shomaresh_show_temp);
